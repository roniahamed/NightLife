...
class RegisterView(generics.CreateAPIView):
    ...
    @extend_schema(summary="Register a new user", description="Creates an inactive user and sends a 4-digit OTP to email.")
...
class VerifyOTPView(APIView):
    ...
    @extend_schema(summary="Verify OTP", description="Verifies the 4-digit OTP. On success, activates the account and returns JWT tokens.")
...
class ResendOTPView(APIView):
    ...
    @extend_schema(summary="Resend Registration OTP", description="Resends the 4-digit OTP to the user's email if the account is not yet verified.")
    def post(self, request):
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            try:
...
class ForgotPasswordView(APIView):
    ...
    @extend_schema(summary="Forgot Password", description="Generates and sends a 4-digit OTP to reset the password.")
    def post(self, request):
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            try:
...
class VerifyResetOTPView(APIView):
    ...
    @extend_schema(summary="Verify Reset OTP", description="Verifies OTP for password reset and returns a reset token.")
    def post(self, request):
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            try:
...
class ResetPasswordView(APIView):
    ...
    @extend_schema(summary="Reset Password", description="Uses the reset token to set a new password.")
    def post(self, request):
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            try:
...
class ChangePasswordView(APIView):
    ...
    @extend_schema(summary="Change Password", description="Change password using old password.")
    def post(self, request):
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            user = request.user
...
class ProfileView(generics.RetrieveUpdateAPIView):
    ...
    @extend_schema(summary="Get User Profile", description="Retrieves the current authenticated user profile.")
    def get(self, request, *args, **kwargs):
        return super().get(request, *args, **kwargs)

    @extend_schema(summary="Update User Profile", description="Updates the current user profile, including latitude/longitude location.")
    def put(self, request, *args, **kwargs):
        return super().put(request, *args, **kwargs)

    @extend_schema(summary="Partial Update User Profile")
